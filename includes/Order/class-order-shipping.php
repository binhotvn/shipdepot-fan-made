...
class Ship_Depot_Order_Shipping
{
     ...
     */
    public static function calculate_shipping_fee(
        $is_cod,
        $is_insurance,
        $insurance_amount,
        $list_packages,
        ...
        $result = Ship_Depot_Helper::http_post($url_API, $dataInput, [
     ...
     * @return Ship_Depot_Courier_Response[]|bool
     */
    public static function calculate_modify_shipping_fee(
        $list_fees_from_api,
        $is_cod,
        $total_item_price_wo_shipping,
        $total_item_qty,
        $list_items
    ) {
        $str_fee_modify = get_option("sd_setting_fee");
        if (!Ship_Depot_Helper::check_null_or_empty($str_fee_modify)) {
            ...
            Ship_Depot_Logger::wrlog(
                "[calculate_modify_shipping_fee] fee_modify: " .
                    print_r($fee_modify, true)
            );
            if (
                $fee_modify->IsActive &&
                count($fee_modify->ListFeeMarkups) > 0
            ) {
                foreach (
                    $fee_modify->ListFeeMarkups
                    as $fee_group_condition_obj
                ) {
                    $fee_group_condition = new Ship_Depot_Fee_Markup(
                        $fee_group_condition_obj
                    );
                    $satisfy = true;
                    ...
                    Ship_Depot_Logger::wrlog(
                        "[calculate_modify_shipping_fee] fee_group_condition active: " .
                            print_r($fee_group_condition, true)
                    );
                    Ship_Depot_Logger::wrlog(
                        "[calculate_modify_shipping_fee] Check time"
                    );
                    //Check time apply
                    if ($fee_group_condition->TimeApply->Type == "period") {
                        if (
                            Ship_Depot_Helper::check_null_or_empty(
                                $fee_group_condition->TimeApply->FromDate
                            ) &&
                            Ship_Depot_Helper::check_null_or_empty(
                                $fee_group_condition->TimeApply->ToDate
                            )
                        ) {
                            $satisfy = false;
                            continue;
                            ...
                            Ship_Depot_Logger::wrlog(
                                "[calculate_modify_shipping_fee][Date from empty] date_to_format: " .
                                    print_r($date_to_format, true)
                            );
                            Ship_Depot_Logger::wrlog(
                                "[calculate_modify_shipping_fee][Date from empty] current_date_format: " .
                                    print_r($current_date_format, true)
                            );
                            ...
                            $current_date_format = date("d-m-Y", time());
                            Ship_Depot_Logger::wrlog(
                                "[calculate_modify_shipping_fee][Date to empty] date_from_format: " .
                                    print_r($date_from_format, true)
                            );
                            Ship_Depot_Logger::wrlog(
                                "[calculate_modify_shipping_fee][Date to empty] current_date_format: " .
                                    print_r($current_date_format, true)
                            );
                            ...
                            $current_date_format = date("d-m-Y", time());
                            Ship_Depot_Logger::wrlog(
                                "[calculate_modify_shipping_fee] date_from_format: " .
                                    print_r($date_from_format, true)
                            );
                            Ship_Depot_Logger::wrlog(
                                "[calculate_modify_shipping_fee] date_to_format: " .
                                    print_r($date_to_format, true)
                            );
                            Ship_Depot_Logger::wrlog(
                                "[calculate_modify_shipping_fee] current_date_format: " .
                                    print_r($current_date_format, true)
                            );
                            if (
                                $current_date_format < $date_from_format ||
                                $current_date_format > $date_to_format
                            ) {
                                $satisfy = false;
                                continue;
                            }
                    ...
                    Ship_Depot_Logger::wrlog(
                        "[calculate_modify_shipping_fee] Check condition"
                    );
                    //Check condition
                    if (count($fee_group_condition->ListOrderConditions) > 0) {
                        foreach (
                            $fee_group_condition->ListOrderConditions
                            as $condition_obj
                        ) {
                            $condition = new Ship_Depot_Fee_Markup_Order_Condition(
                                $condition_obj
                            );
                            ...
                            switch ($condition->Type) {
                                case "total_amount":
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] Check condition total_amount"
                                    );
                                    $total_from = $condition->FromValue;
                                    $total_to = $condition->ToValue;
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] total_from: " .
                                            print_r($total_from, true)
                                    );
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] total_to: " .
                                            print_r($total_to, true)
                                    );
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] total_item_price_wo_shipping: " .
                                            print_r(
                                                $total_item_price_wo_shipping,
                                                true
                                            )
                                    ...
                                    if ($total_to == -9999) {
                                        if (
                                            ...
                                            Ship_Depot_Logger::wrlog(
                                                "[calculate_modify_shipping_fee] satisfy = false"
                                            );
                                            $satisfy = false;
                                        }
                                    } else {
                                ...
                                case "item_existed":
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] Check condition item_existed"
                                    );
                                    $satisfy = false;
                                    $str_list_items = $condition->FixedValue;
                                    $list_sku = explode(",", $str_list_items);
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] list_sku: " .
                                            print_r($list_sku, true)
                                    );
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] list_items: " .
                                            print_r($list_items, true)
                                    );
                                ...
                                case "item_quantity":
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] Check condition item_quantity"
                                    );
                                    $total_qty_from = $condition->FromValue;
                                    $total_qty_to = $condition->ToValue;
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] total_qty_from: " .
                                            print_r($total_qty_from, true)
                                    );
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] total_qty_to: " .
                                            print_r($total_qty_to, true)
                                    );
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] total_item_qty: " .
                                            print_r($total_item_qty, true)
                                    );
                                    if ($total_qty_to == -9999) {
                                        ...
                                        if (
                                            $total_item_qty < $total_qty_from ||
                                            $total_item_qty > $total_qty_to
                                        ) {
                                            Ship_Depot_Logger::wrlog(
                                                "[calculate_modify_shipping_fee] satisfy = false"
                                            );
                                ...
                                case "payment_cod":
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] Check condition payment_cod"
                                    );
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] is_cod: " .
                                            print_r($is_cod, true)
                                    );
                                    if (!$is_cod) {
                                        $satisfy = false;
                                ...
                                case "payment_transfer":
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] Check condition payment_transfer"
                                    );
                                    Ship_Depot_Logger::wrlog(
                                        "[calculate_modify_shipping_fee] is_cod: " .
                                            print_r($is_cod, true)
                            ...
                            if (!$satisfy) {
                                Ship_Depot_Logger::wrlog(
                                    "[calculate_modify_shipping_fee] satisfy = false => break"
                                );
                                break;
                            }
                        }
                    ...
                    //Check service
                    if (
                        $fee_group_condition->CourierApply->Type == "customize"
                    ) {
                        foreach (
                            $fee_group_condition->CourierApply->ListCouriers
                            as $courApply_obj
                        ) {
                            $courApply = new Ship_Depot_Fee_Markup_Courier_Service(
                                $courApply_obj
                            );
                            ...
                            foreach (
                                $courApply->ListServicesSelected
                                as $servApply
                            ) {
                                $is_modify = false;
                                    ...
                                    if (
                                        ...
                                        foreach (
                                            ...
                                            if (
                                                ...
                                                //Set fee
                                                (new Ship_Depot_Order_Shipping())->set_shipping_fee_modify(
                                                    $fee_group_condition,
                                                    $service
                                                );
                                                $is_modify = true;
                        ...
                        foreach ($list_fees_from_api as $courier) {
                            foreach ($courier->ListServices as $service) {
                                //Set fee
                                $service = (new Ship_Depot_Order_Shipping())->set_shipping_fee_modify(
                                    $fee_group_condition,
                                    $service
                                );
                            }
        ...
        Ship_Depot_Logger::wrlog(
            "[calculate_modify_shipping_fee] list_fees_from_api: " .
                print_r($list_fees_from_api, true)
        );
        return $list_fees_from_api;
    }
    ...
    {
        Ship_Depot_Logger::wrlog(
            '[set_shipping_fee_modify] $service bf: ' . print_r($service, true)
        );
        $service->ShopMarkupShippingFee->IsActive = true;
        $service->ShopMarkupShippingFee->MarkupOption =
            $fee_group_condition->Option;
        if ($fee_group_condition->Option->Type == "fixed_fee") {
            $service->ShopMarkupShippingFee->ShippingFeeTotal = intval(
                $fee_group_condition->Option->Value
            );
        } elseif ($fee_group_condition->Option->Type == "depend_shipdepot") {
            switch ($fee_group_condition->Option->SubType) {
                case "inc_percent":
                    $service->ShopMarkupShippingFee->ShippingFeeTotal =
                        $service->ShipDepotMarkupShippingFee->ShippingFeeTotal +
                        ($service->ShipDepotMarkupShippingFee
                            ->ShippingFeeTotal *
                            $fee_group_condition->Option->Value) /
                            100;
                    break;
        ...
        //
        $service->ShopMarkupShippingFee->ShippingFeeTotal = round(
            $service->ShopMarkupShippingFee->ShippingFeeTotal
        );
        if ($service->ShipDepotMarkupShippingFee->ShippingFeeTotal != 0) {
            Ship_Depot_Logger::wrlog(
                "[set_shipping_fee_modify] ShopMarkupShippingFee->ShippingFeeTotal: " .
                    print_r(
                        $service->ShopMarkupShippingFee->ShippingFeeTotal,
                        true
                    )
            ...
            $rate =
                ...
                $service->ShipDepotMarkupShippingFee->ShippingFeeTotal;
            Ship_Depot_Logger::wrlog(
                "[set_shipping_fee_modify] rate: " . print_r($rate, true)
            );
            $service->ShopMarkupShippingFee->ShippingFeeNet = floor(
                $service->ShipDepotMarkupShippingFee->ShippingFeeNet * $rate
            );
            $service->ShopMarkupShippingFee->InsuranceFee = floor(
                $service->ShipDepotMarkupShippingFee->InsuranceFee * $rate
            );
            $service->ShopMarkupShippingFee->CODFee = floor(
                $service->ShipDepotMarkupShippingFee->CODFee * $rate
            );
            $remain_amount =
                $service->ShopMarkupShippingFee->ShippingFeeTotal -
                $service->ShopMarkupShippingFee->ShippingFeeNet -
                $service->ShopMarkupShippingFee->InsuranceFee -
                $service->ShopMarkupShippingFee->CODFee;
            if ($remain_amount != 0) {
                if ($service->ShipDepotMarkupShippingFee->OtherFees != 0) {
                    ...
                    if ($service->ShipDepotMarkupShippingFee->CODFee != 0) {
                        ...
                        $service->ShipDepotMarkupShippingFee->InsuranceFee != 0
                    ) {
        ...
        Ship_Depot_Logger::wrlog(
            '[set_shipping_fee_modify] $service aft: ' . print_r($service, true)
        );
    }
     ...
     */
    public static function get_shipping_stations(
        $city,
        $district,
        $ward_isn,
        $courier_isn
        ...
        Ship_Depot_Logger::wrlog(
            "[Ship_Depot_Order_Shipping][get_shipping_stations] result: " .
                print_r($result, true)
        );
        if ($result->Code >= 0) {
            $list_stations = [];
    ...
    public static function cancel_shipping(
        WC_Order $order,
        bool $update_status = true
    ): Ship_Depot_ResultDTO {
        $shop_api_key = get_option("sd_api_key");
        $data_input = [];
        $ship_info = json_decode(
            Ship_Depot_Helper::GetOrderMetadata($order, "sd_ship_info", true)
        );
        Ship_Depot_Logger::wrlog(
            "[cancel_shipping] ship_info: " . print_r($ship_info, true)
        );
        $data_input["tracking_number"] = isset($ship_info->TrackingNumber)
            ? $ship_info->TrackingNumber
            : "";
        //
        $selected_courier = Ship_Depot_Helper::GetOrderMetadata(
            $order,
            "sd_selected_courier",
            true
        );
        ...
        Ship_Depot_Logger::wrlog(
            "[cancel_shipping] data_input: " . print_r($data_input, true)
        );
        if (
            Ship_Depot_Helper::check_null_or_empty(
            ...
            return new Ship_Depot_ResultDTO(
                -1,
                __("Đơn hàng chưa có vận đơn.", "ship-depot-translate")
            );
        }

        if (
            Ship_Depot_Helper::check_null_or_empty(
                $data_input["selected_courier"]
            )
        ) {
            return new Ship_Depot_ResultDTO(
                -1,
                __(
                    "Không xác định được đơn vị vận chuyển để hủy.",
                    "ship-depot-translate"
                )
            );
        }

        $url = SHIP_DEPOT_HOST_API . "/Shipping/CancelShipping";
        $rs = Ship_Depot_Helper::http_post($url, $data_input, [
            "ShopAPIKey" => $shop_api_key,
        ]);
        Ship_Depot_Logger::wrlog(
            "[cancel_shipping] result call API: " . print_r($rs, true)
        );
        if ($rs->Code > 0) {
            $result = $rs->Data;
            Ship_Depot_Logger::wrlog(
                "[cancel_shipping] result: " . print_r($result, true)
            );
            if ($result) {
                ...
                Ship_Depot_Shipping_Helper::update_cancel_shipping_info(
                    $order,
                    $ship_info->TrackingNumber,
                    $order_note,
                    $update_status
...
add_action(
    ...
    "sd_handle_woo_order_status_changed",
    10,
    3
);
...
function sd_handle_woo_order_status_changed($order_id, $old_status, $new_status)
{
    ...
    Ship_Depot_Logger::wrlog(
        "[sd_handle_woo_order_status_changed] order_id: " .
            print_r($order_id, true)
    );
    Ship_Depot_Logger::wrlog(
        "[sd_handle_woo_order_status_changed] _POST: " . print_r($_POST, true)
    );
    Ship_Depot_Logger::wrlog(
        "[sd_handle_woo_order_status_changed] old_status: " .
            print_r($old_status, true)
    );
    Ship_Depot_Logger::wrlog(
        "[sd_handle_woo_order_status_changed] new_status: " .
    ...
    */
    ...
    if ($new_status == "cancelled") {
        Ship_Depot_Logger::wrlog(
            "[sd_handle_woo_order_status_changed][auto_cancel_shipping] Begin"
        );
        $str_ship_info = Ship_Depot_Helper::GetOrderMetadata(
            $order_id,
            "sd_ship_info",
            true
        );
        Ship_Depot_Logger::wrlog(
            "[sd_handle_woo_order_status_changed][auto_cancel_shipping] str_ship_info: " .
                print_r($str_ship_info, true)
        );
        $order = wc_get_order($order_id);
        $result = Ship_Depot_Order_Shipping::cancel_shipping($order, false);
        if ($result->Code < 0) {
            Ship_Depot_Logger::wrlog(
                "[sd_handle_woo_order_status_changed][auto_cancel_shipping] Cancel shipping of order " .
                    $order_id .
                    " error. Result: " .
                    print_r($result, true)
            );
        ...
        Ship_Depot_Logger::wrlog(
            "[sd_handle_woo_order_status_changed][auto_cancel_shipping] End"
        );
    }

    //Auto create ship
    if (
        sanitize_key($_POST["post_type"]) == "shop_order" ||
        sanitize_key($_POST["post_type"]) == "woocommerce_page_wc-orders" ||
        sanitize_key($_POST["action"]) == "editpost" ||
        sanitize_key($_POST["action"]) == "edit_order"
    ) {
        Ship_Depot_Logger::wrlog(
            "[sd_handle_woo_order_status_changed] from edit order => Not create shipping auto "
        );
        return;
    }

    $order_from_checkout = Ship_Depot_Helper::GetOrderMetadata(
        $order_id,
        "sd_from_fe",
        true
    );
    if (
        Ship_Depot_Helper::check_null_or_empty($order_from_checkout) ||
        $order_from_checkout == "false"
    ) {
        Ship_Depot_Logger::wrlog(
            "[sd_handle_woo_order_status_changed] order not from checkout => Not create shipping auto "
        );
        return;
    }
    ...
    Ship_Depot_Logger::wrlog(
        "[sd_handle_woo_order_status_changed][auto_create_shipping] auto_create_ship: " .
            $auto_create_ship
    );
    if ($auto_create_ship == "yes" || $auto_create_ship == "1") {
        $stt_auto_create_ship = get_option("sd_status_auto_create_shipping");
        Ship_Depot_Logger::wrlog(
            "[sd_handle_woo_order_status_changed][auto_create_shipping] stt_auto_create_ship: " .
                $stt_auto_create_ship
        );
        if (str_contains($stt_auto_create_ship, ",")) {
            $statuses = explode(",", $stt_auto_create_ship);
        } else {
            $statuses = [$stt_auto_create_ship];
        }
        $new_status = "wc-" . $new_status;
        Ship_Depot_Logger::wrlog(
            "[sd_handle_woo_order_status_changed][auto_create_shipping] new_status: " .
                $new_status
        );
        if (in_array($new_status, $statuses)) {
            ...
            if (!Ship_Depot_Helper::check_null_or_empty($str_ship_info)) {
                ...
                if (
                    ...
                    Ship_Depot_Logger::wrlog(
                        "[sd_handle_woo_order_status_changed][auto_create_shipping] TrackingNumber empty => Create shipping"
                    );
                    save_shipping_to_order_meta_data($order_id);
                    $rs = create_ship($order_id, true);
                    if ($rs) {
                        Ship_Depot_Helper::UpdateOrderMetadata(
                            $order_id,
                            "sd_not_create_ship",
                            json_encode(false)
                        );
                    ...
                    Ship_Depot_Logger::wrlog(
                        "[sd_handle_woo_order_status_changed][auto_create_shipping] TrackingNumber existed: " .
                            $ship_info->TrackingNumber .
                            " => Not create shipping"
                    );
                }
                ...
                Ship_Depot_Logger::wrlog(
                    "[sd_handle_woo_order_status_changed][auto_create_shipping] str_ship_info = null => Create shipping"
                );
                save_shipping_to_order_meta_data($order_id);
                $rs = create_ship($order_id, true);
                if ($rs) {
            ...
            Ship_Depot_Logger::wrlog(
                "[sd_handle_woo_order_status_changed][auto_create_shipping] order_status not existed stt_auto_create_ship => Not create shipping"
            );
        }
    } else {
        Ship_Depot_Logger::wrlog(
            "[sd_handle_woo_order_status_changed][auto_create_shipping] auto_create_ship = false => Not create shipping "
        );
    }
}
...
function save_shipping_to_order_meta_data($order_id)
{
    ...
    if (
        !isset($_POST["sd_order_detail_nonce"]) ||
        !wp_verify_nonce($_POST["sd_order_detail_nonce"], "sd_order_detail")
    ) {
        return $order_id;
    }
    ...
    if (Ship_Depot_Helper::check_null_or_empty($shop_api_key)) {
        Ship_Depot_Logger::wrlog(
            "[save_shipping_to_order_meta_data] API Key is empty => cannot save "
        );
        return false;
    }
    ...
    Ship_Depot_Helper::UpdateOrderMetadataWOSave(
        $order,
        "sd_list_package_size",
        $json_packages
    );
    ...
    Ship_Depot_Logger::wrlog(
        "[sd_submit_data_and_save_to_order_meta_data] json_selected_shipping data: " .
            print_r($json_selected_shipping, true)
    );
    Ship_Depot_Helper::UpdateOrderMetadataWOSave(
        $order,
        "sd_selected_shipping",
    ...
    Ship_Depot_Helper::UpdateOrderMetadataWOSave(
        $order,
        "sd_customer_pay_shipping",
        json_encode($cus_pay_ship)
    );

    //list_items
    $get_items = $get_data->get_items();
    Ship_Depot_Logger::wrlog(
        "[sd_submit_data_and_save_to_order_meta_data] get_items: " .
            print_r($get_items, true)
    );
    $list_items = [];
    ...
    if (function_exists("sd_save_wc_order_other_fields")) {
        ...
        Ship_Depot_Logger::wrlog(
            "[sd_submit_data_and_save_to_order_meta_data] unhook."
        );
    ...
    Ship_Depot_Logger::wrlog(
        "[sd_submit_data_and_save_to_order_meta_data] order save."
    );
    $order->save();
    ...
    if (function_exists("sd_save_wc_order_other_fields")) {
        ...
        Ship_Depot_Logger::wrlog(
            "[sd_submit_data_and_save_to_order_meta_data] re-hook."
        );
...
function create_ship($order_id, bool $from_checkout)
{
    $order = wc_get_order($order_id);
    Ship_Depot_Logger::wrlog(
        "[create_ship] get_post_meta: " .
            print_r(Ship_Depot_Helper::GetOrderMetadata($order, "", true), true)
    );
    //List package sizes
    $list_package_size = [];
    $json_list_package_size = Ship_Depot_Helper::GetOrderMetadata(
        $order,
        "sd_list_package_size",
        true
    );
    Ship_Depot_Logger::wrlog(
        "[create_ship] json_list_package_size: " . $json_list_package_size
    );
    ...
    //Sender
    $json_sender_info = Ship_Depot_Helper::GetOrderMetadata(
        $order,
        "sd_sender_info",
        true
    );
    Ship_Depot_Logger::wrlog(
        "[create_ship] json_sender_info: " . $json_sender_info
    );
    ...
    //Sender storage
    $sender_storage = Ship_Depot_Helper::GetOrderMetadata(
        $order,
        "sd_sender_storage",
        true
    );
    Ship_Depot_Logger::wrlog(
        "[create_ship] sender_storage: " . $sender_storage
    );
    //Receiver
    $json_rcv = Ship_Depot_Helper::GetOrderMetadata(
        $order,
        "sd_receiver",
        true
    );
    ...
    //Insurance
    $json_insurance = Ship_Depot_Helper::GetOrderMetadata(
        $order,
        "sd_insurance",
        true
    );
    Ship_Depot_Logger::wrlog(
        "[create_ship] json_insurance: " . $json_insurance
    );
    $insurance = new Ship_Depot_Insurance();
    ...
    Ship_Depot_Logger::wrlog(
        "[create_ship] selected_courier: " . $selected_courier
    );
    //Selected Shipping
    $json_selected_shipping = Ship_Depot_Helper::GetOrderMetadata(
        $order,
        "sd_selected_shipping",
        true
    );
    Ship_Depot_Logger::wrlog(
        "[create_ship] json_selected_shipping: " . $json_selected_shipping
    );
    $selected_shipping = new Ship_Depot_Shipping_Fee_Response();
    if (!Ship_Depot_Helper::check_null_or_empty($json_selected_shipping)) {
        $selected_shipping = new Ship_Depot_Shipping_Fee_Response(
            json_decode($json_selected_shipping)
        );
    } else {
        return false;
    }
    ...
    //Shipping notes
    $shipping_notes = Ship_Depot_Helper::GetOrderMetadata(
        $order,
        "sd_shipping_notes",
        true
    );
    Ship_Depot_Logger::wrlog(
        "[create_ship] shipping_notes: " . $shipping_notes
    );
    //Customer pay shipping
    $str_cus_pay_ship = Ship_Depot_Helper::GetOrderMetadata(
        $order,
        "sd_customer_pay_shipping",
        true
    );
    Ship_Depot_Logger::wrlog(
        "[create_ship] str_cus_pay_ship: " . $str_cus_pay_ship
    );
    $cus_pay_ship =
        Ship_Depot_Helper::check_null_or_empty($str_cus_pay_ship) ||
        $str_cus_pay_ship != "true"
            ? false
            : true;
    ...
    foreach ($order_items as $item) {
        ...
        if ($product_image != false) {
            Ship_Depot_Logger::wrlog(
                "[create_ship] item_image: " . $product_image[0]
            );
        ...
        $it->Length = Ship_Depot_Helper::ConvertToShipDepotDimension(
            $product->get_length()
        );
        $it->Width = Ship_Depot_Helper::ConvertToShipDepotDimension(
            $product->get_width()
        );
        $it->Height = Ship_Depot_Helper::ConvertToShipDepotDimension(
            $product->get_height()
        );
        $it->Weight = Ship_Depot_Helper::ConvertToShipDepotWeight(
            $product->get_weight()
        );
        //
        $regular_price = $product->get_regular_price();
        $item_regular_price_total +=
            floatval($regular_price) * $item->get_quantity();
    ...
    //cod failed info
    $json_cod_failed_info = Ship_Depot_Helper::GetOrderMetadata(
        $order,
        "sd_cod_failed_info",
        true
    );
    Ship_Depot_Logger::wrlog(
        "[create_ship] json_cod_failed_info: " . $json_cod_failed_info
    );
    $cod_failed_info = new Ship_Depot_COD_Failed();
    if (!Ship_Depot_Helper::check_null_or_empty($json_cod_failed_info)) {
        $cod_failed_info = new Ship_Depot_COD_Failed(
            json_decode($json_cod_failed_info)
        );
    }
            ...
            foreach ($setting_courier as $cour_obj) {
                ...
                if ($cour->CourierID == "GHN") {
                    ...
                    Ship_Depot_Helper::UpdateOrderMetadata(
                        $order_id,
                        "sd_ship_from_station",
                        json_encode($ship_from_station, JSON_UNESCAPED_UNICODE)
                    );
                }
            }
        }
    ...
    Ship_Depot_Logger::wrlog(
        "[create_ship] order_created_date string: " .
            print_r($order_created_date->__toString(), true)
    );
    Ship_Depot_Logger::wrlog(
        "[create_ship] ship_from_station: " . print_r($ship_from_station, true)
    );
    ...
    if ($rs->Code > 0) {
        $order_note = __(
            "Vận đơn tạo thành công. Mã vận đơn",
            "ship-depot-translate"
        );
        if ($from_checkout) {
            $order_note = __(
                "Vận đơn được tạo tự động thành công. Mã vận đơn",
                "ship-depot-translate"
            );
        }
        ...
        if (
            !Ship_Depot_Helper::check_null_or_empty($send_tracking_code) &&
            $send_tracking_code == "yes"
        ) {
            $customer_notification_notes = __(
                "Đơn hàng quý khách đang được giao hàng với mã vận đơn %s",
                "ship-depot-translate"
            );
            $customer_notification_notes = sprintf(
                $customer_notification_notes,
                $rs->Data->TrackingNumber
            );
        ...
        Ship_Depot_Helper::UpdateOrderMetadata(
            $order_id,
            "sd_ship_info",
            json_encode($rs->Data, JSON_UNESCAPED_UNICODE)
        );
        Ship_Depot_Helper::UpdateOrderMetadata(
            $order,
            "sd_from_fe",
        ...
        $order_note = __(
            "Vận đơn được tạo tự động thất bại. Lý do:",
            "ship-depot-translate"
        );
        $order_note = $order_note . " " . $rs->Msg;
        $order->add_order_note($order_note);
