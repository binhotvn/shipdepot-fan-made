    ...
    class Ship_Depot_Helper
    {
        ...
        public static function get_data_from_checkbox($data): bool
        {
            if (isset($data)) {
                ...
                return "1" === $data || "yes" === $data || "on" === $data
                    ? true
                    : false;
            } else {
                return false;
        ...
        public static function check_null_or_empty($valueCheck)
        {
            ...
            if (is_null($valueCheck) || $valueCheck == "") {
                return true;
            }
            return false;
        }
            ...
            if (!self::check_null_or_empty($phone)) {
                ...
                if ($phone_length > 3 && $phone_length <= 6) {
                    $format =
                        substr($phone, 0, 3) .
                        "-" .
                        substr($phone, 3, $phone_length - 3);
                } elseif ($phone_length > 6) {
                    $format =
                        substr($phone, 0, 3) .
                        "-" .
                        substr($phone, 3, 3) .
                        "-" .
                        substr($phone, 6, $phone_length - 6);
                }
        ...
        public static function http_get($url, $header_input = [])
        {
            Ship_Depot_Logger::wrlog("[http_get] url: " . $url);
            Ship_Depot_Logger::wrlog(
                "[http_get] header_input: " . print_r($header_input, true)
            );
            ...
            Ship_Depot_Logger::wrlog(
                "[http_get] headers: " . print_r($header, true)
            );
            ...
            $result = new stdClass();
            Ship_Depot_Logger::wrlog(
                "[http_get] http_code: " . print_r($http_code, true)
            );
            Ship_Depot_Logger::wrlog(
                "[http_get] output: " . print_r($output, true)
            );
            if (is_wp_error($response)) {
                ...
                $result->Data = "";
            } elseif (!$output || $http_code != 200) {
                Ship_Depot_Logger::wrlog(
                    "[http_get] HTTP Error or Output error."
                );
                $result->Code = -1001;
                $result->Message =
                    "HTTP GET error. Error message: Connection Failure." .
                        $http_code !=
                    0
                        ? "HTTP Code = " . $http_code
                        : "";
            ...
            Ship_Depot_Logger::wrlog(
                "[http_get] result: " . print_r($result, true)
            );
            return $result;
        }
        /**
         ...
         */
        public static function __extend_http_request_timeout($timeout)
        {
            return 60; // seconds
        }
        public static function http_post(
            $url,
            $body_input = [],
            $header_input = []
        ) {
            ...
            Ship_Depot_Logger::wrlog(
                "[http_post] json_body_input: " .
                    print_r(json_encode($body_input), true)
            );
            Ship_Depot_Logger::wrlog(
                "[http_post] header_input: " . print_r($header_input, true)
            );
            $header = [];
            $header["Expect"] = "";
            foreach ($header_input as $key => $value) {
            ...
            Ship_Depot_Logger::wrlog(
                "[http_post] headers: " . print_r($header, true)
            );
            ...
            $result = new stdClass();
            Ship_Depot_Logger::wrlog(
                "[http_post] http_code: " . print_r($http_code, true)
            );
            Ship_Depot_Logger::wrlog(
                "[http_post] output: " . print_r($output, true)
            );
            if (is_wp_error($response)) {
                Ship_Depot_Logger::wrlog(
                    "[http_post] Reponse WP Error. Message: " .
                ...
                Ship_Depot_Logger::wrlog(
                    "[http_post] HTTP Error or Output error."
                );
                Ship_Depot_Logger::wrlog(
                    "[http_post] http_code: " . $http_code
                );
                $result->Code = -1001;
        ...
        public static function ConvertToShipDepotWeight($weight, $unit = "")
        {
            ...
            if ($sd_weight_unit == "g" || $sd_weight_unit == "gram") {
                if ($woo_weight_unit == "g" || $woo_weight_unit == "gram") {
                    return $weight;
                } elseif ($woo_weight_unit == "kg") {
                    return $weight * 1000;
                } elseif ($woo_weight_unit == "lbs") {
                    return $weight * 453.6;
                } elseif ($woo_weight_unit == "oz") {
                    return $weight * 28.35;
                }
            } elseif (
                $sd_weight_unit == "kg" ||
                $sd_weight_unit == "kilogram"
            ) {
                if ($woo_weight_unit == "g" || $woo_weight_unit == "gram") {
                    return $weight / 1000;
                } elseif ($woo_weight_unit == "kg") {
        ...
        public static function ConvertToShipDepotDimension(
            $dimension,
            $unit = ""
        ) {
            ...
            if ($sd_dimen_unit == "cm") {
                ...
                } elseif ($woo_dimen_unit == "cm") {
                    return $dimension;
                } elseif ($woo_dimen_unit == "mm") {
                    return $dimension * 0.1;
                } elseif ($woo_dimen_unit == "in") {
                    return $dimension * 2.54;
                } elseif ($woo_dimen_unit == "yd") {
                    return $dimension * 91.44;
                }
            } elseif ($sd_dimen_unit == "in" || $sd_dimen_unit == "inch") {
                if ($woo_dimen_unit == "m") {
        ...
        {
            Ship_Depot_Logger::wrlog(
                "[CompareData] old_data: " . print_r($old_data, true)
            );
            Ship_Depot_Logger::wrlog(
                "[CompareData] new_data: " . print_r($new_data, true)
            );
            if (
                !$old_data ||
                Ship_Depot_Helper::check_null_or_empty($old_data)
            ) {
        ...
        public static function UpdateOrderMetadata(
            $order_id,
            $meta_key,
            $meta_value,
            $save_after = true
            ...
            Ship_Depot_Logger::wrlog(
                "[UpdateOrderMetadata] order_id: " . print_r($order_id, true)
            );
            Ship_Depot_Logger::wrlog(
                "[UpdateOrderMetadata] meta_key: " . print_r($meta_key, true)
            );
            Ship_Depot_Logger::wrlog(
                "[UpdateOrderMetadata] meta_value: " .
        ...
        public static function UpdateOrderMetadataWOSave(
            $order,
            $meta_key,
            $meta_value
        ) {
            Ship_Depot_Logger::wrlog(
                "[UpdateOrderMetadataWOSave] meta_key: " .
                    print_r($meta_key, true)
            );
            Ship_Depot_Logger::wrlog(
                "[UpdateOrderMetadataWOSave] meta_value: " .
                    print_r($meta_value, true)
            );
            Ship_Depot_Logger::wrlog(
                "[UpdateOrderMetadataWOSave] update_meta_data."
            );
            $order->update_meta_data($meta_key, $meta_value);
        }

        public static function GetOrderMetadata(
            $order_id_or_order,
            $meta_key = "",
            $single = true
        ) {
